...
def load_graph_web(lnetd_web_url,lnetd_web_user,lnetd_web_password):
    ...
    r = session.get(lnetd_web_url, auth=requests.auth.HTTPBasicAuth(lnetd_web_user, lnetd_web_password))
    if not r.status_code == 200:
        raise ValueError(f"Failed to get devices from LnetD HTTP error code {r.status_code}")
    resp = r.json()
...
def generate_link_number(lnetd_links):
    ...
    while i < len(lnetd_links):
        ...
        elif lnetd_links[i]['source'] == lnetd_links[i-1]['source'] and lnetd_links[i]['target'] == lnetd_links[i-1]['target']:
