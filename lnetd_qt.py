...
class Ui_MainWindow(object):
    ...
    def update_linktable(self):
        ...
        self.LinkTable.setHorizontalHeaderLabels(['Source','Target', 'Metric', 'Util', 'Capacity', 'Local IP', 'Remote IP', 'DOWN', 'ON SPF', 'LINK NR'])
        ...
        for row_number , row_data in enumerate(self.graph.nodes,0):
            for interface in row_data.interfaces:
                ...
                self.LinkTable.setItem(row_number,0,QtWidgets.QTableWidgetItem(str(row_data.label)))
    ...
    def load_topology_json(self):
        ...
        if path != "":
            try:
                ...
                QMessageBox.critical(self.centralwidget, "Error!", "Can't read binary files!")
            except ValueError as e:
                print(e)
                QMessageBox.critical(
                    self.centralwidget, "Error!", "The weights of the graph are not numbers!"
                )
    ...
    def load_topology_lnetd(self):
        if not self.lnetd_web_url or not self.lnetd_web_user or not self.lnetd_web_password:
            QMessageBox.critical(
                    self.centralwidget, "Error!", "LnetD WEB url,user and password must be set before importing, WIP"
                )
        else:
            try:
                self.graph = load_graph_web(self.lnetd_web_url,self.lnetd_web_user,self.lnetd_web_password)
                node_list = self.graph.get_nodes()
                self.scene.clear()
                for node in node_list:
                    for n in node.interfaces:
            ...
            try:
                with open(path, "r") as file:
                    ...
                    for demand in demands['demands']:
                        ...
                        self.graph.add_demand(source=source_node,target=target_target,demand=demand_value)
            except Exception as e:
    ...
    def sceneAddNode(self, scene, event):
        rtr_label = "RTR-" + "" * (len(self.graph.nodes) // 26) + chr(65 + len(self.graph.nodes) % 26)
    ...
    def scenedeleteNode(self,scene,nodeItem):
        ...
        all_scene_links = [ item for item in self.scene.items() if isinstance(item, Link) ]
    ...
    def sceneInterfaceChange(self,scene,interfaceItem):
        ...
        self.change_link.ui.setupUi(self.change_link,interfaceItem.link)
    ...
    def sceneAddInterface(self,scene,nodeItem):
        if len(self.scene.selectedItems()) == 2:
            ...
            target_item = [ item for item in self.scene.selectedItems() if item != nodeItem]
            target_node = target_item[0].node #self.graph.get_node_based_on_label('nl-p13-ams')
            subnet = random.sample(range(0,210),3)
            ip1 = '{}.{}.{}.{}'.format(*subnet+[1])
            ip2 = '{}.{}.{}.{}'.format(*subnet+[2])
            interface_source_target = Interface(target=target_node,metric=10,local_ip=ip1,util=0,capacity=1000,remote_ip=ip2,linknum=1)
            interface_target_source = Interface(target=source_node,metric=10,local_ip=ip2,util=0,capacity=1000,remote_ip=ip1,linknum=1)
            #append to graph
    ...
    def setupUi(self, MainWindow):
        ...
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.legend_widget.sizePolicy().hasHeightForWidth())
        ...
        self.red_legend = QtWidgets.QPushButton(self.legend_widget)
        self.red_legend.setEnabled(False)
        self.red_legend.setAutoFillBackground(True)
        ...
        self.orange_legend.setStyleSheet("background-color: yellow ; color: black")
        ...
        self.l1_model.ui.scene.load_l1_dummy_topology.connect(self.load_l1_topology_dummy)
        ...
        self.load_topology =  QtWidgets.QPushButton(text="Load Topology", clicked=self.load_topology_json)
        self.load_btn_layout.addWidget(self.load_topology, 0, 0, 1, 1)

        self.load_demands = QtWidgets.QPushButton(text="Load Demands", clicked=self.load_demands_json) #self.widget)
        ...
        self.slider = QtWidgets.QSlider(self.static_demands_tab,
        ...
        self.add_demads_btn = QtWidgets.QPushButton(self.static_demands_tab,clicked=self.deploy_static_demand)
        ...
        self.LinkTable.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        ...
        self.DemandTable.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.show_interfaces.setText(_translate("MainWindow", "Show Network Interfaces"))
        self.show_network_demands.setText(_translate("MainWindow", "Show Network Demands"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.load_data_tab), _translate("MainWindow", "Load Data"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.static_demands_tab), _translate("MainWindow", "Static Demands"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.model_inf_tab), _translate("MainWindow", "Model Information"))
        #self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_3), _translate("MainWindow", "Reports"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_5), _translate("MainWindow", "Network Links"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.demand_tab), _translate("MainWindow", "Network Demands"))
    ...
    def apply_icons(self,state):
        ...
        all_scene_links = [ item for item in self.scene.items() if isinstance(item, Rectangle) ]
                    ...
                    for circuit in data:
                        ...
                        for interface in interface_map:
                            if all_graph_interface.get(interface):
                                circuit_obj.interfaces.append(all_graph_interface.get(interface))
    ...
    def set_lnetd_web(self):
        ...
        self.lnetd_web.ui.setupUi(self.lnetd_web,self.lnetd_web_url,self.lnetd_web_password,self.lnetd_web_user)
        self.lnetd_web.show()
