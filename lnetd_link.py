...
class Link(QtWidgets.QGraphicsLineItem):
    ...
    def contextMenuEvent(self,event):
        ...
        if not self.link._failed and action == fail_interface:
            ...
            if scene is not None:
                scene.handleInterfaceActionChange(self, 'this is a message from Node Down GraphicsItem ')
        elif action == delete_interface:
            if scene is not None:
                scene.handleInterfaceActionDelete(self, 'this is a message from Node Down GraphicsItem ')
    ...
    def _generate_bounding_rect(self):
        ...
        new_rec = QRectF(center_of_rec_x - 10 ,center_of_rec_y - 10 ,20,20).normalized();
        return new_rec
