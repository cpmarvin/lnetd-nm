...
# PyQt5
from PyQt5.QtCore import Qt, QSize, QTimer, QPointF, QRectF, QMetaObject, QRect , QCoreApplication, QPoint, QLineF, pyqtSignal
...
class L1Model(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        for vertex in self.vertex_positions:
            #print('vertex in mouse',vertex , pos)
            if (
                ...
                abs(vertex[0][0] - pos[0]) < 3
                and abs(vertex[0][1] - pos[1]) < 3
            ):

                #pressed_vertex = vertex[1]
                pressed_vertex = vertex
                #print('did i catch vertex',pressed_vertex)
        ...
        if event.button() == Qt.LeftButton:
            ...
            if pressed_node is not None:
                ...
                #TODO fail circuit
                self.circuit_click(pressed_vertex[3],event)
            else:
                cmenu = QMenu(self)
                load_l1_topology = cmenu.addAction("Load L1 Topology")
                action = cmenu.exec_(self.mapToGlobal(event.pos()))
                if action == load_l1_topology:
                    self.load_dummy()
            pass
    ...
    def circuit_click(self,pressed_vertex,event):
        cmenu = QMenu(self)
        if pressed_vertex._failed:
            unfail_circuit = cmenu.addAction("Circuit UP")
        else:
            fail_circuit = cmenu.addAction("Circuit DOWN")
        action = cmenu.exec_(self.mapToGlobal(event.pos()))
        if not pressed_vertex._failed and action == fail_circuit:
            pressed_vertex.failCircuit()
            self.update_demands.emit('oh well run the reports if you must')
        elif pressed_vertex._failed and action == unfail_circuit:
            pressed_vertex.unfailCircuit()
            self.update_demands.emit('oh well run the reports if you must')
    ...
    def draw_vertex(self, n1: L1Node, n2: L1Node, label: str , link_num: float, painter, circuit):
        ...
        if self.graph :
            ...
            self.vertex_positions.append((mid, (start, end), 'dummy', circuit))
            #print( 'self.vertex_positions add' ,self.vertex_positions)
